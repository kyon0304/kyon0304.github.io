...
别人只能看到黑白的地方，乘坐架构电梯（可以参考作者的另一篇文章 [Architect Elevator](https://architectelevator.com/)）的人可以看到不同程度的灰色。当考虑系统设计时，他们会意识到，对于像使用锁定这种公共的属性，并不是二元的选择。两个系统不会简单的，就像你不会被某个产品完全锁定或完全未锁定。这两种属性都有很多方面。举例来说，使用锁定可以分解为很多不同维度：
